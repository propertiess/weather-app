...
const AutoCityWeather = () => {
    ...
    const openDetails = () => {
        dispatch(fetchCurrentWeather(`https://api.openweathermap.org/data/2.5/weather?lat=${Math.floor(latitude!)}&lon=${Math.floor(longitude!)}&units=metric&lang=ru&appid=b4753d97985882ce8169158916a467ec`))
